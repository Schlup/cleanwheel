...
const MyProfile = () => {
  ...
  //Tem que adicionar uma chamada de business no login também
  const { user, business, token } = useSelector((state) => state.auth);
  ...
  useEffect(() => {
    async function checkOwnerStatus() {
      try {
        ...
        setIsOwner(response.data); // Aqui sim você pega o `data` corretamente
      } catch (err) {
        console.error(err);
